...
@Component({
  selector: 'app-addgrupos',
  templateUrl: './addgrupos.page.html',
  styleUrls: ['./addgrupos.page.scss'],
})
export class AddgruposPage implements OnInit {
  ...
  constructor(private navCtrl: NavController,
    ...
    { 
      ...
      this.slideOneForm = formBuilder.group({
        grupo: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z0-9]*'), Validators.required])]
      })
  ...
  goBack() {let resultado: any ;
    ...
    this.navCtrl.navigateRoot('/menu/groups');    
  ...
  AddSubmit(form: NgForm) {        
   ...
   console.log("validation" ,this.slideOneForm.status)
      ...
      this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spManutancaoGruposIonic","DataJson", form.value).then(res => {
        let resultado: any = res;
        try {
          if (resultado.length == 0) {
            //nenhum modulo encontrado
