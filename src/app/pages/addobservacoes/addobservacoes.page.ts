...
@Component({
  selector: 'app-addobservacoes',
  templateUrl: './addobservacoes.page.html',
  styleUrls: ['./addobservacoes.page.scss'],
})
export class AddobservacoesPage implements OnInit {
  public    form: NgForm;
  ...
  constructor(private navCtrl: NavController,
    ...
    public formBuilder: FormBuilder) {  this.slideOneForm = formBuilder.group({
      obs: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z0-9,.+]*'), Validators.required])]
    })
  }
  public nome:any[];
  ...
  goBack() {let resultado: any ;
    ...
    this.navCtrl.navigateRoot('/menu/observacoes');    
...
this.alertService.presentAlert({pTitle:'Smtt-sl',pSubtitle:'Erro',pMessage:'a senha e a senha de confirmação não são as'} );
      ...
      this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spInsertObservacoesIonic","DataJson", form.value).then(res => {
        let resultado: any = res;
        try {
          if (resultado.length == 0) {
            //nenhum modulo encontrado
