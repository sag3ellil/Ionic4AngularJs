...
@Component({
  selector: 'app-addbairros',
  templateUrl: './addbairros.page.html',
  styleUrls: ['./addbairros.page.scss'],
})
export class AddbairrosPage implements OnInit {
  public    form: NgForm;
  ...
  constructor(private navCtrl: NavController,
    ...
    { 
      ...
      this.slideOneForm = formBuilder.group({
        bairro: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z0-9,.+]*'), Validators.required])]
      })
...
this.alertService.presentAlert({pTitle:'ValParaiso',pSubtitle:'Erreur',pMessage:'a senha e a senha de confirmação não são as'} );
      ...
      this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spManutancaoBairrosIonic","DataJson", form.value).then(res => {
        let resultado: any = res;
        try {
          if (resultado.length == 0) {
            //nenhum modulo encontrado
