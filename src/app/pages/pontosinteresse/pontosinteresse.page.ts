...
@Component({
  selector: 'app-pontosinteresse',
  templateUrl: './pontosinteresse.page.html',
  styleUrls: ['./pontosinteresse.page.scss'],
})
...
public s:any;
  ...
  mapa(event,form)
  {
   ...
   console.log("validation" ,this.slideOneForm.status)
    ...
    {
      console.log("Invalid")
      this.alertService.presentAlert({pTitle:'Smtt-sl',pSubtitle:'Erreur :',pMessage:' Verifique o formulário,você precisa colocar data  !'} );
    ...
    else
    {
      ...
      this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spConsultaMapeamento2Ionic","DataJson", params2).then(res => {
        let resultado: any = res;
        try {
          if (resultado.length == 0)
           {
        ...
        {       
          if(this.submite[i].longitude!=""&&this.submite[i].latitude!="")
