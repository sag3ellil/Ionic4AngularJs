...
@Component({
  selector: 'app-usodosistema',
  templateUrl: './usodosistema.page.html',
  styleUrls: ['./usodosistema.page.scss'],
})
export class UsodosistemaPage implements OnInit {
  public    form: NgForm;
  ...
  pesquisar(form){
    ...
    console.log("form",form.value)
       ...
       console.log("validation" ,this.slideOneForm.status)
        if(this.slideOneForm.value.data1==null )
        {
          console.log("Invalid")
        ...
        else
        {
           ...
           console.log("d1",d1)
   let ano=   d1.substring(0,4);
   let mes=   d1.substring(5,7);
   console.log("ano",ano)
   console.log("mes",mes)
          ...
          console.log("form",form.value);
          this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spConsultasUsoDoSystemaIonic","DataJson", params2).then(res => {
            let resultado: any = res;
            try {
              if (resultado.length == 0)
               {
