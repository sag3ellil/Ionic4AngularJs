...
@Component({
  ...
  templateUrl: './consultausuario.page.html',
  styleUrls: ['./consultausuario.page.scss'],
})
    ...
    this.Authorizer.QueryStoreProc1('ConsultaSeg',"", params1).then(res => {
      ...
      try {
        if (resultado.length == 0) {
          ...
          this.alertService.presentAlert({ pTitle: 'Smtt-sl', pSubtitle: 'Menu', pMessage: 'Nenhum seg encontrado!' });
        } else {
          //mostro os módulos
        ...
        this.alertService.presentAlert({ pTitle: 'Smtt-sl', pSubtitle: 'Menu', pMessage: 'Nenhum seg!' });
      }
    });
  }
...
console.log("items   :",items)
 this.g=items
  ...
  public u:number=this.codigo;
  saveUsuario(items)
  {
    console.log("items",items)
    if (items.detail.value=="")
     this.u=0
     else
     this.u=this.codigo
  ...
  pesquisar(form){
    ...
    console.log("form",form.value)
       ...
       console.log("validation" ,this.slideOneForm.status)
        ...
        {
          console.log("Invalid")
          this.alertService.presentAlert({pTitle:'Smtt-Sl',pSubtitle:'Erro :',pMessage:'Favor preencher os campos necessários'} );
        ...
        else
        {
          ...
          console.log("@codusuario",this.u)
          if(this.u== undefined)
              form.value.usuario= this.codigo
              else 
              form.value.usuario= this.u
          ...
          let params2={
            ...
            this.Authorizer.QueryStoreProcNgFormExec('executeBouraoui',"spConsultasConsulta3Ionic","DataJson", params2).then(res => {
            let resultado: any = res;
            try {
              if (resultado.length == 0)
               {
